...
val ChartDarkPrimaryText = Color(0xFFEBEBEB)
...
fun LineChart(
    ...
    Canvas(modifier = modifier) {
        ...
        val spacePerHour = (size.width - spacing) / data.size + 10
        /*
        ...
        if (showYLabels) {
            val textPaint = Paint().apply {
                color = ChartDarkPrimaryText.toArgb()
            ...
            drawContext.canvas.nativeCanvas.apply {
                drawText(
                    "Макс ${upperValue.yLabel.orEmpty()}",
                    size.width - 16.dp.toPx(),
                    0 + 8.dp.toPx(),
                    textPaint
                )
                drawText(
                    "Мин ${lowerValue.yLabel.orEmpty()}",
                    size.width - 16.dp.toPx(),
